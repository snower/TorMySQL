...
class ConnectionPool(object):
    ...
    def __init__(self, *args, **kwargs):
        self._max_connections = kwargs.pop(
            "max_connections") if "max_connections" in kwargs else 32
        self._idle_seconds = kwargs.pop(
            "idle_seconds") if "idle_seconds" in kwargs else 7200
        self._wait_connection_timeout = kwargs.pop(
            "wait_connection_timeout") \
            if "wait_connection_timeout" in kwargs else 8
        self._debug_connection_used = kwargs.pop(
            "debug_connection_used") \
            if "debug_connection_used" in kwargs else False
    ...
    def connection_connected_callback(self, future, connection_future):
        if connection_future._exc_info is None:
            ...
            if self._wait_connections and \
               self._connections_count - 1 < self._max_connections:
                ...
                if wait_time >= self._wait_connection_timeout:
                    IOLoop.current().add_callback(
                        wait_future.set_exception, WaitConnectionTimeoutError(
                            "Wait connection timeout, used time %.2fs." % wait_time))
                else:
                    IOLoop.current().add_callback(
                        self.init_connection, wait_future)
    ...
    def init_connection(self, future):
        ...
        self._used_connections[id(connection)] = connection
        IOLoop.current().add_future(connection_future,
                                    lambda connection_future: self.connection_connected_callback(
                                        future, connection_future))

        if self._idle_seconds > 0 and not self._check_idle_callback:
            IOLoop.current().add_timeout(
                time.time() + min(self._idle_seconds, 60),
            ...
            try:
                ...
                if connection not in self._connections:
                    ...
                    raise ConnectionNotUsedError(
                        "Connection is not used, you maybe close wrong connection.")
    ...
    def continue_next_wait(self, connection):
        ...
        while self._wait_connections:
            ...
            if wait_time >= self._wait_connection_timeout:
                self._wait_connection_timeout_futures.append(
                    (wait_future, wait_time))
                continue
    ...
    def do_wait_future_exception_timeout(self):
        ...
        while self._wait_connection_timeout_futures:
            ...
            ioloop.add_callback(wait_future.set_exception, WaitConnectionTimeoutError(
                "Wait connection timeout, used time %.2fs." % wait_time))
    ...
    def close_connection(self, connection):
        try:
            ...
            raise ConnectionUsedError(
                "Connection is used, you can not close it.")
    ...
    def connection_close_callback(self, connection):
        try:
            ...
            try:
                ...
                log.get_log().warning(
                    "Close unknown Connection %s.", connection)
        if self._close_future and \
           not self._used_connections and not self._connections:
    ...
    def close(self, timeout=None):
        ...
        if self._used_connections:
            if timeout:
                def on_timeout():
                    if self._closed and \
                       self._close_future and not self._close_future.done():
                        close_future, self._close_future = self._close_future, None
                        close_future.set_exception(ConnectionPoolUsedError(
                            "Connection pool is used, you must wait all query is finish."))
        ...
        while len(self._wait_connections):
            ...
            if wait_time >= self._wait_connection_timeout:
                IOLoop.current().add_callback(
                    future.set_exception, WaitConnectionTimeoutError(
                        "Wait connection timeout, used time %.2fs." % wait_time))
            else:
                IOLoop.current().add_callback(future.set_exception,
    ...
    def check_idle_connections(self):
        ...
        if not self._closed and (self._connections or self._used_connections):
            IOLoop.current().add_timeout(
                min(next_check_time, now + 60), self.check_idle_connections)
        else:
            self._check_idle_callback = False

    def __str__(self):
        return "%s <%s,%s>" % (
            super(ConnectionPool, self).__str__(),
            len(self._connections), len(self._used_connections))
